...
public class CommentRepayServiceImpl implements CommentRepayService {
    ...
    public ResponseResult loadCommentRepay(CommentRepayDto dto) {
        ...
        //需要查询当前评论中哪些数据被点赞了
        List<String> idList = repayList.stream().map(x -> x.getId()).collect(Collectors.toList());
        ...
        if(CollectionUtils.isEmpty(apCommentRepayLikes)){
            return ResponseResult.okResult(repayList);
        }
        ...
        repayList.forEach(x->{
            ...
            for (ApCommentRepayLike apCommentRepayLike : apCommentRepayLikes) {
                if(x.getId().equals(apCommentRepayLike.getCommentRepayId())){
                    vo.setOperation(ApCommentRepayLike.OP_LIKE);
                    break;
                }
            }
            resultList.add(vo);
    ...
    public ResponseResult saveCommentRepayLike(CommentRepayLikeDto dto) {
        ...
        if(apCommentRepay != null && dto.getOperation() == 0){
            ...
            //删除评论点赞
            Query query = Query.query(Criteria.where("commentRepayId").is(apCommentRepay.getId()).and("authorId").is(userId));
